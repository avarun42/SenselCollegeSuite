...
def findGrid (x,y):
    if (x>0 and x<57.5 and y>0 and y<60):
        return 0
    if (x>57.5 and x<115 and y>0 and y<60):
        return 1
    if (x>115 and x<172.5 and y>0 and y<60):
        return 2
    if (x>172.5 and x<230 and y>0 and y<60):
        return 3
    if (x>0 and x<57.5 and y>60 and y<120):
        return 4
    if (x>57.5 and x<115 and y>60 and y<120):
        return 5
    if (x>115 and x<172.5 and y>60 and y<120):
        return 6
    return 7
    ...
    while not exit_requested: 
        ...
        for c in contacts:
            ...
            if c.type == sensel.SENSEL_EVENT_CONTACT_INVALID:
                ...
                event = "error";
            grid = findGrid(c.x_pos_mm,c.y_pos_mm)
            print("Contact ID %d, event=%s, mm coord: (%f, %f), force=%d, " 
                  "major=%f, minor=%f, orientation=%f,grid=%d" % 
                  (c.id, event, c.x_pos_mm, c.y_pos_mm, c.total_force, 
                   c.major_axis_mm, c.minor_axis_mm, c.orientation_degrees,grid), end="\r\n")
